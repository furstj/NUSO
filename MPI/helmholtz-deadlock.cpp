...
/*
  Solution of u - u_xx = f with periodic boundary conditions

  NOTE: Extremely bad communication pattern - potential deadlock!
 */
...
void update_ghost_zones(double* u, int N, int rank, int ncpu) {
  ...
  int right = (rank+1) % ncpu;
  int left  = (rank>0 ? rank-1 : ncpu-1);

  MPI_Send( &(u[N]), 1, MPI_DOUBLE, right, 0, MPI_COMM_WORLD);
  MPI_Send( &(u[1]), 1, MPI_DOUBLE, left, 0, MPI_COMM_WORLD);

  MPI_Recv( &(u[0]), 1, MPI_DOUBLE, left, 0, MPI_COMM_WORLD, &s);
  MPI_Recv( &(u[N+1]), 1, MPI_DOUBLE, right, 0, MPI_COMM_WORLD, &s);
}
